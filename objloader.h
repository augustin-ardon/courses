...
public:
    ...
    bool intersect(std::vector<Vector> vertices, const Ray &r,  Vector& localN, Vector& localP, double& localt) {
        localN = cross(vertices[vtxj] - vertices[vtxi], vertices[vtxk] - vertices[vtxi]).normalize();
        localt = dot(vertices[vtxk] - r.C, localN) / dot(r.u, localN);
        if (localt < 0) return false;

        localP = r.C + r.u * localt;
        // vérification que le point P est dans le triangle
        Vector u = vertices[vtxj] - vertices[vtxi];
        Vector v = vertices[vtxk] - vertices[vtxi];
        Vector w = localP - vertices[vtxi];
...
public:
    ...
    bool intersect(const Ray& r, Vector& N, Vector& P, double& t) {
        ...
        for (int i = 0; i < indices.size(); ++i) {
            ...
            double localt;
            bool local_intersection = indices[i].intersect(vertices, r, localN, localP, localt);
